...
void print_system_report() {
    ...
    if (__end_of_heap == 0) {
        heap_available = (uint32_t)&_HeapSize;
    }
...
int _write(__attribute__((unused)) int file, char* ptr, int len) {
    LPUART_WriteBlocking(LPUART1, (const uint8_t*)ptr, len);
    return len;
}
...
void operator delete(void* p, __attribute__((unused)) unsigned int length) {
    free(p);
}
