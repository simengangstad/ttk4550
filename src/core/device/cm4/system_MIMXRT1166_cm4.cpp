...
void SystemInit(void) {
    ...
    SCB->CPACR |= ((3UL << 10 * 2) |
                   (3UL << 11 * 2)); /* set CP10, CP11 Full Access */
    ...
    RTWDOG3->CS    = (uint32_t)((RTWDOG3->CS) & ~RTWDOG_CS_EN_MASK) |
    ...
    RTWDOG4->CS    = (uint32_t)((RTWDOG4->CS) & ~RTWDOG_CS_EN_MASK) |
                  RTWDOG_CS_UPDATE_MASK;
#endif /* (DISABLE_WDOG) */
    ...
    if (0U == (LMEM->PCCCR & LMEM_PCCCR_ENCACHE_MASK)) {
        /* set command to invalidate all ways, and write GO bit to initiate
         * command */
    ...
    /* When ECC is enabled, SRC->SRSR need to be cleared since only correct SRSR
       value can trigger ROM ECC preload procedure. Save SRSR to SRC->GPR[11] so
       that application can still check SRSR value from SRC->GPR[11]. */
    SRC->GPR[11] = SRC->SRSR;
    /* clear SRSR */
...
void SystemCoreClockUpdate(void) { /* TBD */
}
