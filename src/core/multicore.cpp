...
#define CORE1_BOOT_TIMEOUT (2000U)
    ...
    /**
     * @brief This is an alias to the start address of the TCM of CORE1 (table
     * 3-1 in the reference manual).
     */
...
namespace multicore {
     ...
     */
    static char rpmsg_base[SH_MEM_TOTAL_SIZE]
     ...
     */
    static int32_t endpoint_callback(void* payload,
    ...
    bool initialise() {
        ...
        uint32_t start = clock::ms();

        do {
            status = MCMGR_GetStartupData(&rpmsg_base_address);
        } while (status != kStatus_MCMGR_Success &&
                 (clock::ms() - start < CORE1_BOOT_TIMEOUT));

        if (clock::ms() - start > CORE1_BOOT_TIMEOUT) {
            return false;
        }
        ...
        rpmsg_lite_wait_for_link_up(rpmsg);
